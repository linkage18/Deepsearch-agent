...
from app.api.context import get_thread_context, get_session_context
...
class ToolMonitor:
    ...
    def _emit(
        ...
        # 工具调用日志：仅记录时间戳、事件类型和名称，不记录参数值防敏感泄漏
        if event_type in ("tool_start", "assistant_call", "error", "task_result", "task_cancelled"):
            try:
                session_dir = get_session_context()
                if session_dir:
                    log_path = Path(session_dir) / "tool_calls.log"
                    tool_name = ""
                    if data:
                        tool_name = data.get("tool_name") or data.get("assistant_name") or ""
                    log_line = (
                        f"{datetime.datetime.now().isoformat()}|{event_type}|{tool_name}\n"
                    )
