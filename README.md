...
项目在 LlamaIndex 向量检索基础上，增加了两层精排策略：

**BM25 候选集融合**：向量检索取 `top_k * 3` 个候选片段后，用 `rank_bm25` + `jieba` 在同一候选集上计算 BM25 分数，与向量分数做 RRF 融合（k=60），重新排序后取 top_k。

**MiniLM 轻量重排序**：对融合后的结果用 `sentence-transformers/all-MiniLM-L6-v2`（~80MB）计算 query-doc 余弦相似度做二次排序。

工具返回结果中注入 `source_type: "knowledge_base"` 字段，供主智能体区分信息来源可靠性。

可调参数集中在 `app/config/retrieval_config.py`：
...
**短期记忆**：基于 LangGraph `InMemorySaver` + `thread_id`，同一会话内的多轮对话可复用执行上下文。

**长期记忆**：基于 JSON 文件的轻量键值存储（`app/memory/memory_store.py`），每轮任务结束时自动保存最终回答前 500 字。下次任务启动时，用用户 query 中的关键词子串匹配历史记忆，匹配到则注入 system prompt。不引入向量检索和 confidence 打分，3 篇论文规模下关键词匹配充分够用。

## 🔒 安全控制

**输入层**：FastAPI 入口限制 query 最长 2000 字符，防止 token 耗尽攻击。

**工具层**：`execute_sql_query` 在 SQL 执行前校验前缀，只放行 `SELECT` / `SHOW` / `WITH` / `DESCRIBE` / `EXPLAIN` 开头的只读查询；`DROP` / `DELETE` / `INSERT` 等写入操作在工具层即被拦截。

**审计日志**：每个会话的工具调用和子智能体调用事件自动写入 `output/session_{id}/tool_calls.log`。

## 🎨 前端设计

前端采用 Neo kinpaku（金箔）视觉系统：暖黑漆底色底、kinpaku 金（`#d4a53a`）为主色锚点、铜绿锈（`#5b8c7a`）为辅色，替代原有冷蓝科技风格。色系统一在 CSS 变量层，便于全局调色。

## 📊 检索评测

基于 `docs/papers/_ground_truth.json` 的 6 条 query，对三种检索策略做 Recall@K 和 MRR 对比：
...
3 篇论文的小语料下 Recall 差异不大，但 **MRR 从 0.61 提升到 0.89**（全链路比纯向量提升 45%），说明重排序显著改善了最相关片段的排序位置。

运行评测：`uv run python -m app.evaluation.evaluate`
